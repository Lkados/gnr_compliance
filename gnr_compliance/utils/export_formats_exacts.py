...
def generer_declaration_trimestrielle_exacte(from_date, to_date):
    ...
    try:
        ...
        try:
            mouvements_journaliers = calculer_mouvements_journaliers_reels(
                from_date, to_date
            )
        ...
        for mouvement in mouvements_journaliers:
            # Date
            ws.cell(
                row=row, column=1, value=mouvement.get("date_format", "")
            ).border = thin_border

            # Stock Initial
            ws.cell(
                row=row, column=2, value=mouvement.get("stock_initial", 0)
            ).border = thin_border
            ws.cell(row=row, column=2).number_format = "#,##0"

            # Entrées
            ws.cell(
                row=row, column=3, value=mouvement.get("entrees", 0)
            ).border = thin_border
            ws.cell(row=row, column=3).number_format = "#,##0"

            # Sorties
            ws.cell(
                row=row, column=4, value=mouvement.get("sorties", 0)
            ).border = thin_border
            ws.cell(row=row, column=4).number_format = "#,##0"

            # Stock Final
            ws.cell(
                row=row, column=5, value=mouvement.get("stock_final", 0)
            ).border = thin_border
            ...
            ws.cell(
                row=row, column=7, value=mouvement.get("sorties", 0)
            ).border = thin_border
            ws.cell(row=row, column=7).number_format = "#,##0"

            # AGRICOLE/FORESTIER (VOLUMES RÉELS AVEC ATTESTATION)
            ws.cell(
                row=row, column=8, value=mouvement.get("volume_agricole_reel", 0)
            ).border = thin_border
            ws.cell(row=row, column=8).number_format = "#,##0"

            # Volume agricole (répétition pour clarté)
            ws.cell(
                row=row, column=9, value=mouvement.get("volume_agricole_reel", 0)
            ).border = thin_border
            ...
            ws.cell(
                row=row,
                column=10,
                value=mouvement.get("volume_sans_attestation_reel", 0),
            ).border = thin_border
        ...
        file_name = (
            f"TIPAccEne Arrêté Trimestriel de Stock Détaillé {trimestre_text}.xlsx"
        )
...
def generer_liste_semestrielle_exacte(from_date, to_date):
    ...
    try:
        ...
        if not clients_data:
            return {
                "success": False,
                "message": "Aucun client trouvé pour cette période",
            }
        ...
        headers = [
            [
                "Informations distributeur autorisé ou fournisseur",
                "",
                "Informations client",
                "",
                "Données carburant GNR",
                "",
            ],
            [
                "Raison sociale",
        ...
        try:
            ...
            company_siren = (
                company_doc.tax_id
                if company_doc and hasattr(company_doc, "tax_id")
                else ""
            )
        except:
        ...
        file_name = (
            f"TIPAccEne Liste Semestrielle des Clients Douane {periode_text}.xlsx"
        )
    ...
    try:
        ...
        while current_date <= end_date:
            ...
            if mouvement:
                ...
                volume_sans_attestation_reel = int(
                    mouvement.volume_sans_attestation_reel or 0
                )
                ...
                entrees = (
                    sorties
                ) = volume_agricole_reel = volume_sans_attestation_reel = 0
    ...
    try:
        ...
        return {
            ...
            "resume": {
                ...
                "produits_avec_taux_suspects": len(
                    [p for p in analyse_taux if p.nb_taux_suspects > 0]
                ),
            },
        }
...
def export_donnees_brutes_excel(from_date, to_date):
    ...
    try:
        ...
        for row_idx, donnee in enumerate(donnees, 2):
            ws.cell(
                row=row_idx, column=1, value=donnee.mouvement_id
            ).border = thin_border
            ws.cell(
                row=row_idx, column=2, value=donnee.date_mouvement
            ).border = thin_border
            ws.cell(
                row=row_idx, column=3, value=donnee.type_mouvement
            ).border = thin_border
            ws.cell(
                row=row_idx, column=4, value=donnee.code_produit
            ).border = thin_border
            ...
            ws.cell(
                row=row_idx, column=7, value=donnee.prix_unitaire
            ).border = thin_border
            ws.cell(row=row_idx, column=8, value=donnee.taux_gnr).border = thin_border
            ws.cell(
                row=row_idx, column=9, value=donnee.montant_taxe_gnr
            ).border = thin_border
            ws.cell(row=row_idx, column=10, value=donnee.client).border = thin_border
            ws.cell(
                row=row_idx, column=11, value=donnee.customer_name
            ).border = thin_border
            ws.cell(
                row=row_idx, column=12, value=donnee.custom_n_dossier_
            ).border = thin_border
            ws.cell(
                row=row_idx, column=13, value=donnee.custom_date_de_depot
            ).border = thin_border
            ws.cell(
                row=row_idx, column=14, value=donnee.statut_attestation
            ).border = thin_border
            ws.cell(
                row=row_idx, column=15, value=donnee.reference_document
            ).border = thin_border
            ws.cell(
                row=row_idx, column=16, value=donnee.reference_name
            ).border = thin_border
