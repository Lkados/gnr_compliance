// ==========================================
// FICHIER: gnr_compliance/public/js/purchase_invoice_gnr.js
// INTERFACE pour annuler factures d'achat avec mouvements GNR
// ==========================================
...
frappe.ui.form.on("Purchase Invoice", {
	refresh: function (frm) {
		...
		if (frm.doc.docstatus === 1) {
			add_gnr_cancel_button_purchase(frm);
			check_gnr_links_purchase(frm);
		}
	},
});

function add_gnr_cancel_button_purchase(frm) {
			...
			if (r.message && r.message.length > 0) {
				...
				if (submitted_movements.length > 0) {
					...
					frm.add_custom_button(
						__("🔄 Annuler avec GNR"),
						function () {
							show_gnr_cancel_dialog_purchase(frm, submitted_movements);
						},
						__("Actions")
...
function check_gnr_links_purchase(frm) {
	...
	frappe.call({
		...
		callback: function (r) {
			if (r.message && r.message.length > 0) {
				add_gnr_info_to_dashboard_purchase(frm, r.message);
			}
		},
	});
}

function add_gnr_info_to_dashboard_purchase(frm, movements) {
...
function show_gnr_cancel_dialog_purchase(frm, gnr_movements) {
	...
	const dialog = new frappe.ui.Dialog({
		title: __("🔄 Annulation Facture d'Achat avec GNR"),
		fields: [
			{
				...
				options: `
                    <div class="alert alert-warning">
                        ...
                        <p>Cette facture d'achat est liée à <strong>${gnr_movements.length} mouvement(s) GNR actif(s)</strong> :</p>
		...
		primary_action: function (data) {
			...
			dialog.hide();
			execute_gnr_action_purchase(frm, data.action_type, gnr_movements);
		},
			...
			if (r.message && r.message.success) {
				...
				setTimeout(() => {
					...
					frappe.msgprint({
						...
						message: __(
							"La facture d'achat et tous ses mouvements GNR ont été annulés avec succès."
						),
						indicator: "green",
					});
				}, 2000);
...
function execute_movements_cancel_purchase(frm) {
...
function execute_debug_mode_purchase(frm) {
	frappe.call({
