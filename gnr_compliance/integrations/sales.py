    ...
    try:
        ...
        if hasattr(invoice_doc, 'taxes') and invoice_doc.taxes:
            for tax_row in invoice_doc.taxes:
                if tax_row.description:
                    ...
                    gnr_keywords = ['gnr', 'accise', 'ticpe', 'gazole', 'fioul', 'carburant', 'tipp', 'diesel']
                        ...
                        if item.qty > 0 and tax_row.tax_amount:
                            ...
                            if quantity_in_litres > 0:
                                ...
                                if 0.1 <= taux_calcule <= 50:
                                    frappe.logger().info(f"[GNR] Taux RÉEL trouvé dans taxes facture {invoice_doc.name}: {taux_calcule}€/L (taxe: {tax_row.tax_amount}€ / {quantity_in_litres}L)")
                                    return taux_calcule
        ...
        if hasattr(item, 'gnr_tax_rate') and item.gnr_tax_rate:
            if 0.1 <= item.gnr_tax_rate <= 50:
                frappe.logger().info(f"[GNR] Taux trouvé dans champ item facture: {item.gnr_tax_rate}€/L")
                ...
                if matches:
                    for match in matches:
                        ...
                        if 0.1 <= taux_potentiel <= 50:
                            frappe.logger().info(f"[GNR] Taux trouvé dans termes facture: {taux_potentiel}€/L")
                            return taux_potentiel

        # 4. PRIORITÉ 4: Analyser le nom de l'article pour déduire le type
        category = detect_gnr_category_from_item(item.item_code, getattr(item, 'item_name', ''))
        ...
        frappe.logger().warning(f"[GNR] Aucun taux réel trouvé pour {item.item_code}, utilisation taux par défaut {category}: {default_rate}€/L")
        return default_rate

    except Exception as e:
        frappe.log_error(f"Erreur récupération taux réel pour {item.item_code}: {str(e)}")
        return 0.0
    ...
    try:
        ...
        if item_group in GNR_ITEM_GROUPS:
            ...
            try:
                ...
                frappe.logger().info(f"[GNR] Article {item_code} marqué automatiquement comme GNR (catégorie: {category})")
            except:
                pass
...
def capture_vente_gnr(doc, method):
    ...
    try:
        ...
        frappe.logger().info(f"[GNR] Capture vente avec VRAIS TAUX: {doc.name}, Date: {posting_date}")
        ...
        for item in doc.items:
            ...
            if is_gnr:
                ...
                if not existing:
                    ...
                    prix_unitaire_par_litre = item.rate / (quantity_in_litres / item.qty) if item.qty else item.rate
                    ...
                    try:
                        mouvement.submit()
                        movements_created += 1
                        frappe.logger().info(f"[GNR] Mouvement créé avec TAUX RÉEL: {mouvement.name} - {quantity_in_litres}L à {taux_gnr_reel}€/L = {montant_taxe_reel}€")
        ...
        frappe.log_error(f"Erreur capture GNR vente avec taux réels pour facture {doc.name}: {str(e)}")
        frappe.msgprint(_("Erreur lors de la création des mouvements GNR: {0}").format(str(e)))
...
def capture_achat_gnr(doc, method):
    ...
    try:
        ...
        frappe.logger().info(f"[GNR] Capture achat avec VRAIS TAUX: {doc.name}, Date: {posting_date}")
            ...
            if is_gnr:
                ...
                if not existing:
                    ...
                    try:
                        mouvement.submit()
                        movements_created += 1
                        frappe.logger().info(f"[GNR] Mouvement GNR achat créé avec taux réel {taux_gnr_reel}€/L: {mouvement.name} pour facture {doc.name}")
                    except Exception as submit_error:
        ...
        frappe.log_error(f"Erreur capture GNR achat avec taux réels pour facture {doc.name}: {str(e)}")
    ...
    try:
        ...
        for mouvement in mouvements_suspects:
            try:
                ...
                facture = frappe.get_doc(mouvement.reference_document, mouvement.reference_name)
                ...
                for item in facture.items:
                    if item.item_code == mouvement.code_produit:
                        ...
                        if nouveau_taux and nouveau_taux != mouvement.taux_gnr:
                            ...
                            corriges += 1
                            frappe.logger().info(f"[GNR] Mouvement {mouvement.name} corrigé: {mouvement.taux_gnr} → {nouveau_taux}€/L")
                            break
                        
            except Exception as e:
                frappe.log_error(f"Erreur correction mouvement {mouvement.name}: {str(e)}")
                echecs += 1
        ...
        if analyse:
            ...
            return {
                ...
                "statistiques": {
                    ...
                    "pourcentage_suspects": round((stats.taux_suspects / total) * 100, 1),
                    "pourcentage_avec_facture": round((stats.avec_facture / total) * 100, 1),
